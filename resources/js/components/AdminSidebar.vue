...
<template>
    <div
        v-if="store.isMenuVisible && !store.isLargeScreen"
        class="opacity-50 bg-gray-900 z-150 w-full h-full left-0 top-0 fixed"
        @click="closeMenu"
    ></div>

    <nav
        v-if="(store.isMenuVisible && !store.isLargeScreen) || store.isLargeScreen"
    ...
    >
        ...
        >
            <button
                class="ml-auto flex items-center text-sm text-slate-700"
                @click="closeMenu"
            >
                Close
            </button>
